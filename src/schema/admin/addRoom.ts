...
const addRoomSchema = yup.object().shape({
  ...
  name: yup
    .string()
    .trim()
    .matches(/^(?!\d)[a-zA-Z0-9 ]*$/, "Invalid input")
    .required("Name is required"),
  description: yup.string().trim().required("Description is required"),
  area: yup
    .number()
    .max(1000, "Invalid Area")
    .min(1, "Invalid Area")
  ...
  maxPeople: yup
    ...
    .test('even', 'Maximum no. of People must be an even number', function(value) {
      return value! % 2 === 0;
    })
  ...
  noOfRooms: yup
    ...
    .min(1,"Minimum atleast 1 room required")
